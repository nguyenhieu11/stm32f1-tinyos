 ...
 *
 * Ported from Ian Wilkey's tinyrtos (Cortex-M7) to Cortex-M3.
 * Key change: removed FPU-related code (Cortex-M3 has no FPU).
 */
...
/**
 * SVC_Handler — launches the first task.
 *
 * Restores R4-R11 from the first task's stack, sets PSP,
 * switches Thread mode to use PSP, and performs exception return.
 */
 ...
 *
 * Saves R4-R11 of current task, calls scheduler, restores R4-R11 of next task.
 * No FPU lazy-stacking code (Cortex-M3 has no FPU).
 */
...
/**
 * HardFault_Handler — captures stack frame and forwards to C handler.
 */
...
void rtosk_port_configure_exceptions(void) {
    /* PendSV: lowest priority so it runs after all other ISRs */
    NVIC_SetPriority(PendSV_IRQn, 0xFFU);
    /* SysTick: one level above PendSV */
