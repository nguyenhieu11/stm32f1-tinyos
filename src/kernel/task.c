/**
 * @file    task.c
 * @brief   TinyRTOS task management (minimal — supports kernel core).
 */
...
/* Idle task: runs WFI in a loop (low-power wait for interrupt) */
...
/* Trap: catches tasks that accidentally return */
static void rtosk_task_exit_trap(void) {
    for (;;) {}
}

/* Fill task stack with watermark pattern for overflow detection */
...
/* Count unused stack words (watermark still intact from bottom) */
static uint32_t rtosk_task_get_stack_free_words(rtosk_task_t * task) {
    uint32_t free_words = 0UL;
    while (free_words < RTOSK_TASK_STACK_WORDS &&
           task->stack[free_words] == RTOSK_STACK_WATERMARK) {
        free_words++;
    }
    return free_words;
}
...
/**
 * Build initial Cortex-M exception stack frame for a new task.
 *
 * The frame layout matches what the hardware pushes on exception entry:
 *   SP -> R4-R11 (software-saved by PendSV)
 *         R0-R3, R12, LR, PC, xPSR (hardware-saved on exception entry)
 *
 * When PendSV "restores" this frame for the first time, it loads
 * R4-R11 (all zeros), advances SP, and the hardware pops the rest,
 * jumping to task_func.
 */
static uint32_t * rtosk_task_build_stack(rtosk_task_t * task, rtosk_task_func_t task_func) {
    ...
    *(--sp) = RTOSK_XPSR_T_BIT;                       /* xPSR: Thumb bit set */
    *(--sp) = (uint32_t)task_func;                     /* PC: task entry point */
    *(--sp) = (uint32_t)rtosk_task_exit_trap;          /* LR: safety trap */
    *(--sp) = 0x00000000UL;                            /* R12 */
    *(--sp) = 0x00000000UL;                            /* R3  */
    *(--sp) = 0x00000000UL;                            /* R2  */
    *(--sp) = 0x00000000UL;                            /* R1  */
